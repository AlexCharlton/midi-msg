...
pub trait Write {
    ...
    fn write(&mut self, buf: &[u8]) -> WriteResult<Self> {
        self.write_iter(buf.iter().copied())
    }
    /// Write a slice of data to the writer.
    ///
    /// Should error if not all of the data could be written.
    fn write_iter(&mut self, mut data: impl Iterator<Item = u8>) -> WriteResult<Self> {
...
impl<W: Write> Write for NotSeekable<W> {
    ...
    #[inline]
    fn push(&mut self, _: u8) -> WriteResult<Self> {
        self.as_never()
    }
}
...
impl<'a, W: Write> Write for &'a mut W {
    ...
    #[inline]
    fn write_iter(&mut self, data: impl Iterator<Item = u8>) -> WriteResult<Self> {
        W::write_iter(self, data)
    }
    #[inline]
    fn push(&mut self, b: u8) -> WriteResult<Self> {
        W::push(self, b)
...
impl<'a> Write for Cursor<'a> {
    ...
    #[inline]
    fn write(&mut self, buf: &[u8]) -> WriteResult<Self> {
        let end = self.cur + buf.len();
        if end > self.buf.len() {
            Err(CursorError::OutOfSpace)
        } else {
            self.buf[self.cur..end].copy_from_slice(buf);
            self.cur += buf.len();
            Ok(())
        }
    }
